...
extern "C"
jint Java_com_amse_ys_zip_DeflatingDecompressor_startInflating(JNIEnv *env, jobject thiz) {
	int i;
	for (i = 0; i < SIZE; ++i) {
		if (ourStreams[i] == 0) {
...
extern "C"
void Java_com_amse_ys_zip_DeflatingDecompressor_endInflating(JNIEnv *env, jobject thiz, jint inflatorId) {
...
extern "C"
jlong Java_com_amse_ys_zip_DeflatingDecompressor_inflate(JNIEnv *env, jobject thiz, jint inflatorId, jbyteArray in, jint inOffset, jint inLength, jbyteArray out) {
	if (inflatorId < 0 || inflatorId >= SIZE) {
		return -1;
	}
	...
	const int code = inflate(stream, Z_SYNC_FLUSH); // 调用zlib库的inflate方法
