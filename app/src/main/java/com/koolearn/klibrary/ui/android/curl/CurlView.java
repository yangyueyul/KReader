...
//    private static final int CURL_LEFT = 1; // 左翻转 上一页
//    private static final int CURL_NONE = 0; // 没有翻转
//    private static final int CURL_RIGHT = 2; // 右翻转 下一页
//
//    // Constants for mAnimationTargetEvent.
//    private static final int SET_CURL_TO_LEFT = 1; // 动画向左
//    private static final int SET_CURL_TO_RIGHT = 2; // 动画向右
//
//    // Shows one page at the center of view.
//    public static final int SHOW_ONE_PAGE = 1; // 单屏显示
//    // Shows two pages side by side.
//    public static final int SHOW_TWO_PAGES = 2; // 双屏显示
//
...
//    private PointF mDragStartPos = new PointF(); // 拖动的起始位置
//
//    private boolean mEnableTouchPressure = true; // 页面压缩效果
//    // Bitmap size. These are updated from renderer once it's initialized.
//    private int mPageBitmapHeight = -1; // 初始化大小
//
...
//    private int mViewMode = SHOW_ONE_PAGE; // 默认单页显示
//
//    /**
//     * Default constructor.
//     */
...
////                Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.bg_green);
...
//        mPageLeft = new CurlMesh(100); // 卷曲程度,越大越平滑
//        mPageRight = new CurlMesh(100);
//        mPageCurl = new CurlMesh(100);
...
//                ZLAndroidWidget.Instance().setMyPendingPress(true);
//                ZLAndroidWidget.Instance().setMyScreenIsTouched(true);
//                ZLAndroidWidget.Instance().setMyPressedX(x);
//                ZLAndroidWidget.Instance().setMyPressedY(y);
//
//
...
//                // user is holding the paper to make curl happen. 翻卷到手指按压处
...
//                final boolean isAMove = Math.abs(ZLAndroidWidget.Instance().getMyPressedX() - x) > slop || Math.abs(ZLAndroidWidget.Instance().getMyPressedY() - y) > slop; // 判断是否在移动
//                if (ZLAndroidWidget.Instance().isMyPendingPress()) {
//                    if (isAMove) {
//                        view.onFingerPress(ZLAndroidWidget.Instance().getMyPressedX(), ZLAndroidWidget.Instance().getMyPressedY());
//                        ZLAndroidWidget.Instance().setMyPendingPress(false);
//                    }
//                }
//                if (!ZLAndroidWidget.Instance().isMyPendingPress()) {
...
//                }
//                ZLAndroidWidget.Instance().setMyPendingPress(false);
//                ZLAndroidWidget.Instance().setMyScreenIsTouched(false);
//
...
//
//        // Default curl radius.
//        double radius = mRenderer.getPageRect(CURL_RIGHT).width() / 3; // 默认卷曲半径
